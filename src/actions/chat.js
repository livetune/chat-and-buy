...
export function connectSocket() {
  return (dispatch, state) => {
    const orders = state()
      ...
      .get('orders')
    const ip =
      process.env.NODE_ENV === 'development' ? '192.168.1.108' : 'livetune.xyz'
...
export function cleanNoRead(readId, messageId) {
  return async (dispath, getState) => {
    try {
      const res = await axios.post('/api/chat/cleanNoRead', {
        readId,
        messageId
      })
...
export function getMessageList() {
  return async (dispatch, state) => {
    try {
      const res = await axios.post('/api/chat/getMessageList')
